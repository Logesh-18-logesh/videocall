...
class CameraClient(StreamingClient):
    def __init__(self, host, port, background_image_path, x_res=1024, y_res=576):
        self.__x_res = x_res
        self.__y_res = y_res
        self.__camera = cv2.VideoCapture(0)
        self.model = self.load_model() if background_image_path != "no" else None  # Load model only if not "no"
        self.background_mode = background_image_path  # Store the background mode (either path, "blur", or "no")

        if background_image_path not in ["blur", "no"]:
            self.background_image = Image.open(background_image_path).resize((x_res, y_res))  # Load and resize background image
        else:
            self.background_image = None  # No background image loaded in blur or no-removal mode
    ...
    def make_transparent_foreground(self, pic, mask):
        ...
        new_mask = np.stack([mask]*4, axis=2)
        foreground = np.where(new_mask, alpha_im, np.zeros_like(alpha_im)).astype(np.uint8)
        return foreground
    ...
    def remove_background(self, input_image):
        ...
        foreground = self.make_transparent_foreground(input_image, mask)
        
        return foreground

    def blur_background(self, frame):
        """Apply a blur effect to the background of the frame."""
        blurred_frame = cv2.GaussianBlur(frame, (21, 21), 0)
        return blurred_frame

    def blend_with_background(self, foreground, frame):
        # If blur mode is active, apply blur to the background
        if self.background_mode == "blur":
            background_cv = self.blur_background(frame)
        else:
            # Convert PIL Image to OpenCV format for blending
            background_cv = cv2.cvtColor(np.array(self.background_image), cv2.COLOR_RGB2BGR)

        # Convert foreground to OpenCV format for blending
        foreground_cv = cv2.cvtColor(foreground, cv2.COLOR_RGBA2BGR)

        # Create mask from foreground
        alpha_channel = foreground[:, :, 3] / 255.0  # Normalize alpha channel to [0, 1]

        # Blend images using alpha channel
        for c in range(3):  # For each color channel
            foreground_cv[:, :, c] = (alpha_channel * foreground_cv[:, :, c] + 
                                       (1 - alpha_channel) * background_cv[:, :, c])
    ...
    def _get_frame(self):
        ...
        final_frame = None
        if ret:
            # If background mode is "no", return the original frame
            if self.background_mode == "no":
                cv2.imshow("your Frame", frame)
            else:
                # Convert frame to PIL Image for processing
                input_image = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
                # Remove background and get the processed foreground
                foreground = self.remove_background(input_image)

                # Blend with background image (or apply blur if in blur mode)
                final_frame = self.blend_with_background(foreground, frame)

                # Display the processed frame in a window
                cv2.imshow("your Frame", final_frame)

            # Check for 'q' key press to exit
            if cv2.waitKey(1) & 0xFF == ord('q'):
                self.stop_stream()

            return frame if final_frame is None else final_frame
